...
export function MiniInput({
  ...
  return (
    <label className={cn("bg-zinc-100 rounded-[3px] inline-flex h-5 shrink-0 items-center gap-0.5 pr-2", width, className)}>
      <input
        data-editor-mini
        value={String(value)}
        onChange={(event) => onChange(event.target.value)}
        className="h-5 min-w-0 flex-1 border-0 px-1 text-right font-mono text-[10px] leading-none text-zinc-700 outline-none"
      />
