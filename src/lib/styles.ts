...
export function styleToCss(styles?: StyleProps): CSSProperties {
  ...
  assign(out, "flexWrap", styles.flexWrap);
  assign(out, "flexGrow", styles.flexGrow);
  assign(out, "flexShrink", styles.flexShrink);
  assign(out, "flexBasis", styles.flexBasis);
  assign(out, "aspectRatio", styles.aspectRatio);
  assign(out, "objectFit", styles.objectFit);
  assign(out, "objectPosition", styles.objectPosition);
  assign(out, "backgroundSize", styles.backgroundSize);
  assign(out, "backgroundPosition", styles.backgroundPosition);
  assign(out, "backgroundRepeat", styles.backgroundRepeat);
  assign(out, "gridTemplateColumns", styles.gridTemplateColumns);
  assign(out, "gridTemplateRows", styles.gridTemplateRows);
  assign(out, "gridColumn", styles.gridColumn);
  assign(out, "gridRow", styles.gridRow);
  const translateX = styles.translateX ? `translateX(${styles.translateX})` : "";
  const translateY = styles.translateY ? `translateY(${styles.translateY})` : "";
  const rotate = styles.rotate ? `rotate(${styles.rotate})` : "";
  const scale = styles.scale ? `scale(${styles.scale})` : "";
  const transform = [translateX, translateY, rotate, scale].filter(Boolean).join(" ");
